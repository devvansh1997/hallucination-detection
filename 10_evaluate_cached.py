"""
10_evaluate_cached.py — Cached Factor Matrices + RF/LR/SVM
=============================================================
Saves U_L, U_D to disk after first HOSVD computation; loads from
cache on subsequent runs. Identical classifiers to 09_evaluate_all.
"""
...
def evaluate_one(model_folder: str, dataset: str, idx: int = 1, total: int = 1):
    ...
    # Subsampling: train on 5k stratified subset, test on full validation
    n_sub = min(5000, len(y_train))
    if len(y_train) > n_sub:
        _, X_train_sub, _, y_train_sub = train_test_split(
            X_train_feat, y_train, train_size=n_sub,
            stratify=y_train, random_state=RANDOM_SEED)
        print(f"  Train subset: {n_sub} / {len(y_train)} (stratified)")
    else:
        X_train_sub, y_train_sub = X_train_feat, y_train
        print(f"  Train subset: all {len(y_train)} (no subsampling needed)")
    ...
    # SVM-RBF (on subset, decision_function, test on full)
    ...
    svm_scores = svm.decision_function(X_valid_feat)
    svm_auroc = roc_auc_score(y_valid, svm_scores)
