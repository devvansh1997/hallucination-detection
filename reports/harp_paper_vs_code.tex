...
$^{\ast}$TriviaQA answer-level covers 5 of 6 conditions: joint\_tensor was OOM-killed at 200\,GB.
Since the cell is a maximum it can only rise, and joint\_tensor ranks mid-pack on all three other
datasets (3rd, 4th, 5th), so it would have to break its own pattern to matter.}
\end{table}
...
\begin{itemize}
\item \textbf{TriviaQA is one condition short.} Our answer-level cell there covers 5 of 6:
joint\_tensor was OOM-killed at 200\,GB (it entered the first fold already holding 93\,GB, against
56\,GB for core\_concat and 81\,GB for triple\_concat). Since the cell is a maximum, the $-1.35$
inflation figure is a lower bound.
