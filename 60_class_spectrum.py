...
def run(dataset, model_folder, in_dir, out_dir, stream, n_boot, seed, layers=None):
    ...
    todo = list(range(n_layers)) if layers is None else layers
    ...
    for li in todo:
        ...
        print("    layer %2d  erank truthful %7.2f [%.2f, %.2f]   hallucinated %7.2f [%.2f, %.2f]"
              "   %s" % (li, e0[0], e0[1], e0[2], e1[0], e1[1], e1[2],
                         "SEPARATED" if separated(e0, e1) else "overlapping"), flush=True)
    ...
    verdict = ("no separation at any depth -- the classes do not differ in effective rank, and this "
               "figure should not go in the paper"
               if n_sep == 0 else
               "hallucinated answers use MORE directions at %d/%d depths" % (n_sep, len(rows))
               if all(s > 0 for s in signs) else
               "truthful answers use MORE directions at %d/%d depths" % (n_sep, len(rows))
               if all(s < 0 for s in signs) else
               "separated at %d/%d depths but the SIGN FLIPS across depth -- not a clean story"
               % (n_sep, len(rows)))
    print("\n  VERDICT: %s" % verdict)
    ...
    with open(dst, "w") as f:
        json.dump({"dataset": dataset, "model_folder": model_folder, "stream": stream,
                   ...
                   "layers": rows, "elapsed_seconds": round(time.time() - t0, 1)}, f, indent=2)
    print("  wrote %s" % dst)
    return rows
...
def main():
    ...
    run(a.dataset, a.model_folder, a.in_dir, a.out_dir, a.stream, a.n_boot, a.seed, a.layers)
