"""
18_evaluate_gradient_egte.py — Gradient EGTE (Discrete Derivative)
=====================================================================
Finds the reasoning phase transition via max gradient of layer-wise
energy divergence, centering a 9-layer HOSVD window at the inflection point.
"""
...
def evaluate_one(model_folder: str, dataset: str, idx: int = 1, total: int = 1):
    ...
    # EGTE: gradient-based inflection point
    ...
    grad = torch.diff(delta)                                     # (L-1,)

    # Search middle 70% of gradient (exclude first/last 15%)
    lo = int((total_L - 1) * 0.15)
    hi = int((total_L - 1) * 0.85)
    middle_grad = grad[lo:hi]
    peak_local = int(middle_grad.argmax().item())
    inflection_layer = lo + peak_local

    # Center 9-layer window at inflection point
    start_layer = max(0, inflection_layer - 4)
    end_layer   = min(total_L, start_layer + 9)
    start_layer = end_layer - 9                               # guarantee exactly 9
    X_all = X_all[:, start_layer:end_layer, :]
    window_L = X_all.shape[1]

    print(f" done.  {total_L}L, inflection at layer {inflection_layer} "
          f"(max gradient)")
    print(f"  Gradient EGTE window: layers {start_layer}:{end_layer} "
          f"({window_L} layers)")
