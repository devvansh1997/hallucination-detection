"""
06_evaluate_cropped.py — Mechanistic Layer Cropping + HOSVD
=============================================================
Drops the first 11 layers (0-10) before HOSVD projection to test if
early-layer semantic features dilute the anomaly detection subspace.

Usage:
  python 06_evaluate_cropped.py --model_folder llama-3.1-8b-instruct --dataset triviaqa
"""
...
def evaluate_one(model_folder: str, dataset: str, idx: int = 1, total: int = 1):
    ...
    # Layer cropping: drop first 11 layers (0-10)
    orig_shape = X_all.shape
    X_all = X_all[:, 11:, :]
    crop_shape = X_all.shape
    print(f" done.  {orig_shape[1]}L -> {crop_shape[1]}L, "
          f"{orig_shape[0]} beams, {crop_shape[2]}D")
