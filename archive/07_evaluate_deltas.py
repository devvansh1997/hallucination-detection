"""
07_evaluate_deltas.py — Delta-HOSVD (Layer-wise Increments)
=============================================================
Computes layer-wise deltas before HOSVD projection:
  delta = X[:, 1:, :] - X[:, :-1, :]
Isolates reasoning velocity by subtracting static prompt context
that dominates absolute hidden-state coordinates across all layers.

Usage:
  python 07_evaluate_deltas.py --model_folder llama-3.1-8b-instruct --dataset triviaqa
"""
...
def evaluate_one(model_folder: str, dataset: str, idx: int = 1, total: int = 1):
    ...
    # Delta-HOSVD: layer-wise increments remove static prompt context
    orig_shape = X_all.shape
    X_all = X_all[:, 1:, :] - X_all[:, :-1, :]
    delta_shape = X_all.shape
    print(f" done.  {orig_shape[1]}L -> {delta_shape[1]}L (deltas), "
          f"{orig_shape[0]} beams, {delta_shape[2]}D")
