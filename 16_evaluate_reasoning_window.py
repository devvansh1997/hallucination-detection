"""
16_evaluate_reasoning_window.py — Reasoning Window HOSVD (Layers 15-23)
=========================================================================
Crops to middle layers only, then pure HOSVD (R_L=5, R_D=64).
No HARP filter — isolates the reasoning signal localised in mid-layers.
"""
...
def evaluate_one(model_folder: str, dataset: str, idx: int = 1, total: int = 1):
    ...
    X_all = X_all[:, 15:24, :]
    crop_L = X_all.shape[1]
    print(f" done.  {orig_L}L -> {crop_L}L (reasoning window), "
          f"{X_all.shape[0]} beams, {X_all.shape[2]}D")
